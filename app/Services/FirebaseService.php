...
class FirebaseService
{
    ...
    public static function sendBasicNotification(string $title, string $body, array $devices, array $metadata = [])
    {
        $icon = transform(
            ClientPreference::query()->first(['favicon'])->favicon ?: null,
            fn($favicon) => (string) (Str::of($favicon['proxy_url'])
                ->append('200/200')
                ->append($favicon['image_path'])),
        );

        return (new static)->sendNotification([
            ...
            'data' => array_merge(compact('title', 'body'), $metadata),
        ]);
